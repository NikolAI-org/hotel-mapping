...
class UIDProcessor(BaseProcessor[DataFrame]):
    def __init__(self, name_col: str = "normalized_name", address_col: str = "combined_address",output_col="uid"):
    ...
    def process(self, df: DataFrame, prefix: str = "") -> DataFrame:
        ...
        clean_addr = F.regexp_replace(F.lower(F.col(self.address_col)), r"[^a-z0-9]", "")
        
        return (
            df
            .withColumn(self.output_col, F.sha2(F.concat(F.col("providerId"), clean_name, clean_addr), 256))
        )