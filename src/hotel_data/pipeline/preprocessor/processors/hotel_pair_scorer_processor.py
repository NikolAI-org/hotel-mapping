...
class HotelPairScorerProcessor(BaseChallengeProcessor[DataFrame]):
    ...
    def process(self, challenger_df: DataFrame, anchor_df: DataFrame) -> DataFrame:
        ...
        # ---------------------------------------------------------
        # print("🔍 Running Postal Code Blocker...")
        # postal_blocker = PostalCodeBlocker()
        # postal_pairs = postal_blocker.block(anchor_df, challenger_df)
        # raw_pairs = geo_pairs.unionByName(postal_pairs)
        # ---------------------------------------------------------
        raw_pairs = geo_pairs
        ...
        pairs = unique_pairs.join(a, F.col("uid_i") == F.col("a.uid"), "inner").join(
            b, F.col("uid_j") == F.col("b.uid"), "inner"
        )
        ...
        pairs = pairs.select(
            ...
            F.col("a.combined_address_preprocessed").alias(
                "combined_address_preprocessed_i"
            ),
            F.col("b.combined_address_preprocessed").alias(
        ...
        df_scores = (
            ...
            .withColumn(
                "property_type_score", type_match_udf(F.col("type_i"), F.col("type_j"))
            )
            .withColumn(
                "name_unit_score",
                unit_match_udf(
                    F.col("name_preprocessed_i"), F.col("name_preprocessed_j")
                ),
            )
            .withColumn(
                "address_unit_score",
            ...
            .withColumn(
                ...
                "supplier_score",
                F.when(F.col("providerName_i") == F.col("providerName_j"), 1).otherwise(
                    0
                ),
            )
        )
