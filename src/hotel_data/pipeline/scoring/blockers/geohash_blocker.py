...
class GeoHashBlocker(BaseBlocker):
    def block(self, anchor_df: DataFrame, challenger_df: DataFrame) -> DataFrame:
        ...
        anchor_skinny = anchor_df.select("uid", "geoHash", "contact_address_country_code")
        challenger_skinny = challenger_df.select("uid", "geoHash", "contact_address_country_code")
        ...
        unique_pairs = joined_skinny.select(
            F.col("a.uid").alias("uid_i"),
            F.col("c.uid").alias("uid_j")
