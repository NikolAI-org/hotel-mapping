...
_NAME_UNIT_COMPACT_RE = re.compile(r"\b(\d{1,4}|[ivxlcdm]{1,7})\s*(?:bhk|bed|bedroom|bath|beds|bedrooms|baths|bathrooms)\b")
...
def _extract_name_units(name: str) -> Set[str]:
    ...
    for match in _TYPED_TYPE_FIRST_RE.finditer(text):
        ...
        span = match.span()
        overlap = any(start <= span[0] < end or start < span[1] <= end for start, end in consumed_spans)
