...
class CandidateScorer(BaseProcessor[DataFrame]):
    ...
    def process(self, df: DataFrame, prefix: str = "") -> DataFrame:
        ...
        pairs_with_distance = df.withColumn(
            "geo_distance_km",
            native_haversine(
                "geoCode_lat_i", "geoCode_long_i",
                "geoCode_lat_j", "geoCode_long_j"
            )
        )
        ...
        sbert_df = jaccard_lcs_levenshtein.withColumn(
            ...
            get_cosine_similarity_expr(F.col("normalized_name_embedding_i"), F.col("normalized_name_embedding_j")).cast(
                "float")
        )
        ...
        cols_to_remove = ["norm_phones_i", "norm_phones_j", "norm_faxes_i", "norm_faxes_j"]
