...
class EntityResolutionPipeline:
    ...
    def run(
        ...
        new_hotels_df = (
            manager.read_table(table_hotels)
        ...
        if not has_pairs:
            if not manager._table_exists(table_output):
                ...
                output_df = new_hotels_df.withColumn(
                    "cluster_id", (F.row_number().over(w)).cast("string")
                )
                ...
                max_row = existing_df.select(
                    F.max(F.col("cluster_id").cast("int")).alias("max_id")
                ).collect()[0]
                ...
                output_df = new_hotels_df.withColumn(
                    "cluster_id",
                    (F.row_number().over(w) + F.lit(max_id)).cast("string"),
                )
        ...
        evaluated_pairs_df = self.append_failure_reasons(
            evaluated_pairs_df, self.match_logic_config
        )
        ...
        signal_cols = [
            ...
            F.col("address_line1_score"),
            F.col("address_sbert_score"),
            F.col("star_ratings_score"),
            F.col("postal_code_match"),
            F.col("phone_match_score"),
            F.col("email_match_score"),
            F.col("fax_match_score"),
        ]
        ...
        output_df = self.resolver.cluster(
            new_hotels_df, evaluated_pairs_df, existing_clusters_df
        )
        ...
        # ---------------------------------------------------------
        evaluated_pairs_df = (
            evaluated_pairs_df.join(
                full_mapping.withColumnRenamed("cluster_id", "cluster_id_i"),
                evaluated_pairs_df.uid_i == full_mapping.uid,
                "left",
            )
            .drop("uid")
            .join(
                full_mapping.withColumnRenamed("cluster_id", "cluster_id_j"),
                evaluated_pairs_df.uid_j == full_mapping.uid,
                "left",
            )
            .drop("uid")
        )
        ...
        evaluated_pairs_df = evaluated_pairs_df.withColumn(
            ...
            F.when(
                ...
                & (F.col("cluster_id_i") != F.col("cluster_id_j")),
                True,
            ).otherwise(False),
        )
    ...
    def build_match_expression(self, rule_node: dict) -> F.Column:
        ...
        if "operator" in rule_node:
            ...
            child_exprs = [
                self.build_match_expression(child) for child in rule_node["rules"]
            ]
            if not child_exprs:
                return F.lit(False)
            return functools.reduce(
                lambda x, y: x & y if op == "AND" else x | y, child_exprs
            )
    ...
    def append_failure_reasons(
        self, df: DataFrame, match_logic_config: dict
    ) -> DataFrame:
        ...
        if match_logic_config.get("operator", "").upper() != "AND":
            ...
            return df.withColumn(
                "failed_conditions", F.expr("CAST(array() AS array<string>)")
            )
