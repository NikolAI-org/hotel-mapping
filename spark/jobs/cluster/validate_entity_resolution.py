...
def main():
    if len(sys.argv) < 3:
        ...
        print("Usage: spark-submit validate_entity_resolution.py <audit_s3_path> <clusters_s3_path>")
        sys.exit(1)

    audit_path = sys.argv[1]
    clusters_path = sys.argv[2]

    spark = SparkSession.builder.appName("DataValidation_EntityResolution").getOrCreate()
    ...
    if audit_count == 0:
        ...
        invalid_scores = audit_df.filter((F.col("score") < 0.0) | (F.col("score") > 1.0)).count()
        if invalid_scores > 0: errors.append(f"Audit Log: {invalid_scores} rows have scores outside [0.0, 1.0].")

        valid_statuses = ["MATCHED", "MANUAL_REVIEW", "UNMATCHED"]
        invalid_status = audit_df.filter(~F.col("status").isin(valid_statuses)).count()
        if invalid_status > 0: errors.append(f"Audit Log: {invalid_status} rows have an invalid status.")
    ...
    if cluster_count == 0:
        ...
        if null_clusters > 0: errors.append(f"Clusters: {null_clusters} rows have a null cluster_id.")

        duplicate_uids_df = clusters_df.groupBy("uid").count().filter(F.col("count") > 1)
    ...
    if cluster_count > 0 and audit_count > 0:
        ...
        top_cluster_row = clusters_df.groupBy("cluster_id").count().orderBy(F.desc("count")).first()
        ...
        if top_cluster_row:
            ...
            top_uids = [r["uid"] for r in clusters_df.filter(F.col("cluster_id") == top_cid).select("uid").collect()]
            
            # Find all audit pairs where BOTH hotels are inside this cluster (Internal Edges)
            internal_pairs_df = audit_df.filter(
                F.col("hotel_id").isin(top_uids) & F.col("compared_with_id").isin(top_uids)
            )
            ...
            if internal_count > 0:
                ...
                internal_pairs_df.groupBy("status").count().orderBy("status").show(truncate=False)
                
                print("Note: If you see 'UNMATCHED' or 'MANUAL_REVIEW' here, it proves TRANSITIVITY is working!")
                print("(e.g., A matches B, and B matches C. Therefore A and C are clustered, even if A vs C scored poorly.)")
                ...
                print("No historical comparisons found for these UIDs (They may be singletons or from an older run).")
    ...
    transitivity_df = audit_df.join(c1, audit_df.hotel_id == c1.u1, "inner") \
                              .join(c2, audit_df.compared_with_id == c2.u2, "inner")

    # Find pairs that failed direct matching but share the same cluster_id
    transitive_bridges = transitivity_df.filter(
        (F.col("status") != "MATCHED") & 
        (F.col("cid1") == F.col("cid2"))
    )
    
    bridge_count = transitive_bridges.count()
    print(f"🌉 Found {bridge_count} Transitive Connections across the dataset!")
    if bridge_count > 0:
        transitive_bridges.select("hotel_name", "compared_with_name", "score", "status", "cid1").show(5, truncate=False)
    ...
    if "is_matched" in audit_df.columns and "geo_distance_km" in audit_df.columns:
        ...
        rule_violations = audit_df.filter((F.col("is_matched") == True) & (F.col("geo_distance_km") > 0.5)).count()
        if rule_violations > 0:
            errors.append(f"Rule Engine Failure: {rule_violations} pairs clustered with a distance > 0.5km.")
        else:
        ...
        rejected_df = audit_df.filter((F.col("status") == "MATCHED") & (F.col("is_matched") == False))
        if rejected_df.count() > 0:
            rejected_df.select("hotel_name", "compared_with_name", "score", "geo_distance_km", "is_matched").show(5, truncate=False)
        else:
            print("No high-scoring pairs were rejected.")

        print("\n📈 Borderline Score (MANUAL_REVIEW) but Accepted by Rule Engine:")
        accepted_df = audit_df.filter((F.col("status") == "MANUAL_REVIEW") & (F.col("is_matched") == True))
        if accepted_df.count() > 0:
            accepted_df.select("hotel_name", "compared_with_name", "score", "geo_distance_km", "is_matched").show(5, truncate=False)
        else:
            print("No borderline pairs were auto-accepted.")
            
    else:
        print("⚠️ Skipping Suite 5: 'is_matched' and 'geo_distance_km' columns not found in audit log.")
    ...
    if logic_failures > 0:
        print(f"❌ LOGIC FAILED: Found {logic_failures} pairs that should have matched but didn't.")
