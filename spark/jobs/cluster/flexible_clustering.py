...
class FlexibleClustering:
    ...
    def cluster(
        self,
        new_hotels_df: DataFrame,
        scored_pairs_df: DataFrame,
        existing_clusters_df: DataFrame = None,
    ) -> DataFrame:
        # 1. Collect Edges from Scored Pairs (Only those satisfying the Match Logic)
        matched_pairs = scored_pairs_df.filter(F.col("is_matched") == True).select(
            "uid_i", "uid_j"
        )
        ...
        parent_map = self._driver_side_union_find(
            edges_list, existing_map, new_uids, max_cluster_id
        )

        # 5. Map back to Spark
        cluster_data = [
            (uid, str(cluster_id))
            for uid, cluster_id in parent_map.items()
            if uid in new_uids
        ]
