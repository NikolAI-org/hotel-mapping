...
builder = (
    SparkSession.builder.appName("Delta Lake Transformation")
...
try:
    ...
    df_transformed = (
        df_raw.withColumn("booking_date", to_date(col("booking_date")))
        .withColumn("checkin_date", to_date(col("checkin_date")))
        .withColumn("checkout_date", to_date(col("checkout_date")))
        .withColumn(
            "nights_stayed", datediff(col("checkout_date"), col("checkin_date"))
        )
        .withColumn(
            "booking_lead_time", datediff(col("checkin_date"), col("booking_date"))
        )
        .withColumn(
            "revenue",
    ...
    df_agg = (
        df_verify.groupBy("hotel_name", "city")
        .agg({"booking_id": "count", "revenue": "sum", "nights_stayed": "avg"})
