...
def run_delta_validation_job(**kwargs):
    ...
    spark_submit_cmd = [
        ...
        "--deploy-mode", "client",
        "--name", "Validate-Entity-Resolution",
        "--jars",
    ...
    if result.returncode != 0:
        print("--- SPARK STDERR ---")
        print(result.stderr)
        raise Exception(f"Validation job failed. Check logs for Data Quality errors. Exit code: {result.returncode}")
...
with DAG(
    ...
    validate_delta_task = PythonOperator(
        task_id="run_validation_task", 
        python_callable=run_delta_validation_job
    )